...
def pca_get_probe_units_df(merged_dic, probe, roi_filter=None, brain_region=None, kslabel_filter="both", bc_label_filter=None):
    ...
    if brain_region is not None:
        if 'brain_region' not in df.columns:
            raise ValueError("brain_region filter requested but brain_region column is missing.")
        df = df[df["brain_region"].astype(str) == str(brain_region)].reset_index(drop=True)
...
def verify_units_post_filtering(merged_dic, probes, roi_filter=None, kslabel_filter="both", bc_label_filter=None):
    dfs = []
    errors = []

    for probe in probes:
        try:
            df = pca_get_probe_units_df(
            ...
            df = df.copy()
            df["probe"] = probe  # track provenance for per-region probe counts
            dfs.append(df)

        except Exception as e:
            print(f"Probe {probe}: Error during filtering - {e}")
            errors.append({"probe": probe, "error": str(e)})

    if len(dfs) == 0:
        out = pd.DataFrame(columns=["brain_region", "n_units", "n_probes", "probes", "n_brain_regions_total", "n_units_total"])
        if len(errors) > 0:
            out.attrs["errors"] = errors
        return out

    all_units = pd.concat(dfs, ignore_index=True)

    if "brain_region" not in all_units.columns:
        raise KeyError("verify_units_post_filtering: 'brain_region' column not found in filtered unit dataframe(s).")

    all_units = all_units.dropna(subset=["brain_region"])

    per_region = (
        all_units.groupby("brain_region", dropna=False)
        .agg(
            n_units=("brain_region", "size"),
            n_probes=("probe", "nunique"),
            probes=("probe", lambda x: sorted(set(x))),
        )
        .reset_index()
        .sort_values("n_units", ascending=False, ignore_index=True)
    )

    n_units_total = int(len(all_units))
    n_brain_regions_total = int(per_region.shape[0])

    per_region["n_units_total"] = n_units_total
    per_region["n_brain_regions_total"] = n_brain_regions_total

    if len(errors) > 0:
        per_region.attrs["errors"] = errors  # optional: inspect df.attrs["errors"]
...
# === LEGACY PLOT STYLE OVERRIDES ===

def _legacy_resolve_color_mode(color_mode=None) -> int:
    if color_mode is None:
        color_mode = _EPOCH_PLOT_STYLE_DEFAULTS.get("color_mode", 1)
    return _normalize_color_mode(color_mode)


def _legacy_normalize_condition_token(v: str) -> str | None:
    s = str(v).strip().lower()
...
def _legacy_parse_condition_epoch_label(label, label_mode=None) -> tuple[str, int] | None:
    s = str(label).strip()
    if s == "":
        return None
    mode = None if label_mode is None else str(label_mode).strip().lower().replace("-", "_").replace(" ", "_")
    sl = s.lower()

    if mode == "condition":
        cond = _legacy_normalize_condition_token(sl)
        return None if cond is None else (cond, 0)

    if mode in {"condition_epoch", "conditionepoch"}:
        m = re.match(r"^(baseline|stimulation|washout)(?:_?epoch)?(?:[_\-\s]?(\d+))?$", sl)
        if m:
            cond = _legacy_normalize_condition_token(m.group(1))
            ep = int(m.group(2)) if m.group(2) is not None else 0
            return None if cond is None else (cond, ep)
        cond = _legacy_normalize_condition_token(sl)
        return None if cond is None else (cond, 0)

    # Auto-detect when label_mode is not explicitly condition/condition_epoch.
    m = re.match(r"^(baseline|stimulation|washout)(?:_?epoch)?(?:[_\-\s]?(\d+))?$", sl)
    if m:
        cond = _legacy_normalize_condition_token(m.group(1))
        ep = int(m.group(2)) if m.group(2) is not None else 0
...
def _legacy_label_style_maps(labels, *, color_mode=None, label_mode=None):
    unique_labels = [str(x) for x in dict.fromkeys([str(v) for v in labels])]
    mode = _legacy_resolve_color_mode(color_mode)

    parsed = {lbl: _legacy_parse_condition_epoch_label(lbl, label_mode=label_mode) for lbl in unique_labels}
    all_parsed = len(unique_labels) > 0 and all(v is not None for v in parsed.values())

    markers_cycle = ["o", "^", "s", "D", "P", "X", "v", "<", ">", "*"]
    lines_cycle = ["-", "--", "-.", ":"]
    styles = {}

    # If labels are condition/epoch-compatible, reuse epoch color-mode logic.
    if all_parsed:
        em = pd.DataFrame(
            {
                "condition": [parsed[lbl][0] for lbl in unique_labels],
                "epoch_id": [int(parsed[lbl][1]) for lbl in unique_labels],
            }
        )
        style_ctx, cond_marker, cond_linestyle = _epoch_condition_color_marker_maps(em, color_mode=mode)
        for i, lbl in enumerate(unique_labels):
            cond, ep = parsed[lbl]
            styles[lbl] = {
                "color": _epoch_condition_color_for(style_ctx, cond, ep, default_color="tab:blue"),
                "marker": cond_marker.get(cond, markers_cycle[i % len(markers_cycle)]),
                "linestyle": _epoch_condition_linestyle_for(cond_linestyle, cond),
            }
        return styles, mode

    # Generic label styling when labels are not condition/epoch-like.
    n = max(1, len(unique_labels))
    if mode == 1:
        pal = sns.color_palette("colorblind", max(3, n))
    elif mode == 2:
        pal = sns.color_palette("crest", max(3, n))
    else:
        pal = sns.color_palette("Greys", max(4, n + 1))[1 : n + 1]
        pal = pal[::-1]  # darker first

    for i, lbl in enumerate(unique_labels):
        styles[lbl] = {
            "color": pal[i % len(pal)],
            "marker": markers_cycle[i % len(markers_cycle)],
            "linestyle": lines_cycle[i % len(lines_cycle)],
        }
    return styles, mode


def _legacy_mix_with_white(color, mix=0.35):
    c = np.asarray(mcolors.to_rgb(color), dtype=float)
    mix = float(np.clip(mix, 0.0, 1.0))
    return tuple((1.0 - mix) * c + mix * np.ones(3, dtype=float))


def _legacy_dedupe_legend(handles, labels):
    uniq = {}
    for h, l in zip(handles, labels):
        key = str(l)
...
def pca_plot_trial_level(
    ...
    projections = [(0, 1), (1, 2), (0, 2)]
    style_map, mode = _legacy_label_style_maps(trial_types, color_mode=color_mode, label_mode=label_mode)

    fig, axes = plt.subplots(1, 3, figsize=(13.5, 4.6))
    trial_labels = [str(t) for t in trial_types]
    for ax, (i, j) in zip(axes, projections):
        for k, lbl in enumerate(trial_labels):
            idx = np.asarray(t_type_ind[k], dtype=int)
            if idx.size == 0:
                continue
            style = style_map.get(lbl, {})
            ax.scatter(
                Xp[i, idx],
                Xp[j, idx],
                s=30,
    ...
    main_title = "Legacy Trial-Level PCA" if str(title_prefix).strip() == "" else str(title_prefix)
    fig.suptitle(main_title, fontsize=14, y=0.99)
    if subtitle:
        fig.text(0.5, 0.945, str(subtitle), ha="center", va="center", fontsize=10)
    else:
        fig.text(0.5, 0.945, f"color_mode={mode}", ha="center", va="center", fontsize=10)

    handles, labels = axes[-1].get_legend_handles_labels()
    handles, labels = _legacy_dedupe_legend(handles, labels)
    if len(handles) > 0:
        axes[-1].legend(
            handles,
            labels,
            title=legend_title or ("condition/epoch" if label_mode in {"condition", "condition_epoch"} else "labels"),
            frameon=False,
    ...
    save_path = _ensure_dir(Path(save_root) / "trial_level")
    out = save_path / f"{_sanitize_name(main_title)}.png"
    plt.savefig(out, dpi=250, bbox_inches="tight")
    if show_plots:
        plt.show()
    else:
...
def pca_plot_trajectory(
    ...
    style_map, mode = _legacy_label_style_maps(kept_labels, color_mode=color_mode, label_mode=label_mode)
    fig, axes = plt.subplots(1, 3, figsize=(14.5, 4.8), sharex=True)

    labels = [str(x) for x in kept_labels]
    for comp in range(min(3, Xa_p.shape[0])):
        ax = axes[comp]
    ...
    axes[1].set_xlabel("Time from event (s)")
    main_title = "Legacy Trajectory PCA" if str(title_prefix).strip() == "" else str(title_prefix)
    fig.suptitle(main_title, fontsize=14, y=0.99)
    if subtitle:
        fig.text(0.5, 0.945, str(subtitle), ha="center", va="center", fontsize=10)
    else:
    ...
    save_path = _ensure_dir(Path(save_root) / "trajectory")
    out = save_path / f"{_sanitize_name(main_title)}_trajectory.png"
    plt.savefig(out, dpi=300, bbox_inches="tight")
    if show_plots:
        plt.show()
    else:
...
def plot_probe_stimulus_panel(
    ...
    mode = _legacy_resolve_color_mode(color_mode)
    w_start = WINDOW_START_S if win_start_s is None else float(win_start_s)
    w_end = WINDOW_END_S if win_end_s is None else float(win_end_s)
    b_size = BIN_SIZE_S if bin_size_s is None else float(bin_size_s)
    ...
    spike_times_list = [np.asarray(x, dtype=float) for x in probe_df["spike_times"].values]
    stim_names = sorted(events_df[stim_label_col].astype(str).str.strip().unique().tolist())
    stim_style, _ = _legacy_label_style_maps(stim_names, color_mode=mode, label_mode="stimulus")
    ...
    _base_save_root = Path(save_dir) if save_dir is not None else Path(save_root)
    _main_title = f"Probe {probe} | ROI={roi_filter} | KS={kslabel_filter} | BR={region_label}"
    _sub_title = f"Legacy Stimulus Panel | units={n_units} | color_mode={mode}"

    if panel_per_probe:
        n = len(per_stim)
        nrows = int(np.ceil(n / ncols))
        fig, axes = plt.subplots(nrows, ncols, figsize=(5 * ncols, 3.3 * nrows), sharex=True)
        axes = np.array(axes).reshape(-1)
        ...
        for i, rec in enumerate(per_stim):
            ax = axes[i]
            base_color = stim_style.get(rec["stim_name"], {}).get("color", "tab:blue")
            c1 = base_color
            c2 = _legacy_mix_with_white(base_color, mix=0.30)
            c3 = _legacy_mix_with_white(base_color, mix=0.55)
            ...
            ax.plot(t, x1, lw=2.1, color=c1, label="PC1")
            if x2 is not None:
                ax.plot(t, x2, lw=1.7, color=c2, label="PC2")
            if x3 is not None:
                ax.plot(t, x3, lw=1.4, color=c3, label="PC3")
            ax.axvline(0, color="gray", ls="--", lw=1)
            ax.grid(True, alpha=0.2, linestyle="--", linewidth=0.6)
            ax.set_title(f"{rec['stim_name']} (n={rec['n_events']})", color=base_color)
            ax.set_xlabel("Time (s)")
            ax.set_ylabel("PC value")

        h, l = axes[0].get_legend_handles_labels()
        h, l = _legacy_dedupe_legend(h, l)
        if len(h) > 0:
            axes[0].legend(h, l, title="Components", frameon=False, fontsize=8)

        fig.suptitle(_main_title, y=0.995, fontsize=12)
        fig.text(0.5, 0.96, _sub_title, ha="center", va="center", fontsize=10)
    ...
    if one_stim_per_fig:
        for rec in per_stim:
            ...
            fig, ax = plt.subplots(1, 1, figsize=(6.3, 3.9))
            t = rec["time"]
            x1 = rec["traj"][0]
            x2 = rec["traj"][1] if rec["traj"].shape[0] > 1 else None
            x3 = rec["traj"][2] if rec["traj"].shape[0] > 2 else None
            ...
            h, l = _legacy_dedupe_legend(h, l)
            if len(h) > 0:
                ax.legend(h, l, title="Components", frameon=False, fontsize=8)

            fig.suptitle(_main_title, y=0.995, fontsize=11)
            fig.text(0.5, 0.955, _sub_title, ha="center", va="center", fontsize=9)
            sns.despine()
            plt.tight_layout(rect=[0, 0, 1, 0.93])
...
def plot_probe_selected_stimuli_overlay(
    ...
    for stim_name in selected_stimuli:
        ...
        traj, evr = pca_single_stim_trajectory(trials_stim, n_components=n_components)
        recs.append({"stim": str(stim_name), "n": int(len(stim_times)), "time": t, "traj": traj, "evr": evr})

    if len(recs) == 0:
        print(f"Probe {probe} | region={region_label}: no selected stimuli were plottable.")
        return None

    style_map, _ = _legacy_label_style_maps([r["stim"] for r in recs], color_mode=mode, label_mode="stimulus")
    fig, axes = plt.subplots(1, 3, figsize=(20.5, 4.1), sharex=True)

    for rec in recs:
        style = style_map.get(rec["stim"], {})
        color = style.get("color", "tab:blue")
        ...
        axes[0].plot(tt, x1, lw=2.1, color=color, linestyle=ls, label=f"{rec['stim']} (n={rec['n']})")
        if x2 is not None:
            axes[1].plot(tt, x2, lw=2.1, color=color, linestyle=ls, label=f"{rec['stim']} (n={rec['n']})")
        if x3 is not None:
            axes[2].plot(tt, x3, lw=2.1, color=color, linestyle=ls, label=f"{rec['stim']} (n={rec['n']})")
    ...
    fig.suptitle(
        f"Probe {probe} | ROI={roi_filter} | KS={kslabel_filter} | BR={region_label}",
        y=0.99,
        fontsize=12,
    )
    fig.text(
        0.5,
        0.95,
        f"Legacy Stimulus Overlay | units={n_units} | color_mode={mode}",
        ha="center",
        va="center",
        fontsize=10,
    )
    sns.despine()
    plt.tight_layout(rect=(0.19, 0, 1, 0.92))
